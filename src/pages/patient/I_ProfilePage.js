...
function ProfilePage() {
  ...
  const loadUserData = useCallback(() => {
    ...
    if (user) {
      ...
      if (user.profile_picture) {
        setProfilePreview(`https://oravista-server-474976105474.asia-southeast1.run.app/${user.profile_picture}`);
      }
    }
  }, []);
  ...
  const handleImageUpload = async (e) => {
    ...
    if (file) {
      ...
      try {
        const response = await fetch("https://oravista-server-474976105474.asia-southeast1.run.app/api/upload-profile-picture", {
          method: "POST",
          body: formData,
        });
        ...
        if (response.ok) {
          ...
          setProfilePreview(`https://oravista-server-474976105474.asia-southeast1.run.app/${data.imagePath}`);
        } else {
  ...
  const handleDiscard = () => {
    ...
    setIsEditing(false);
    const user = JSON.parse(localStorage.getItem("user"));
    setProfilePreview(user?.profile_picture ? `https://oravista-server-474976105474.asia-southeast1.run.app/${user.profile_picture}` : null);
  };
